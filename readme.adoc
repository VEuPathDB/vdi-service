...
files to be used with smart editor validation & autocomplete.

[NOTE]
--
Documentation is maintained by version starting with the v1.7.0 release. The
docs for specific tags may be referenced by replacing the version reference or
'latest' in the doc url with the desired tag:

[source, subs=attributes]
{docs-root}/{SERVICE_TAG}/...
--

==== Latest Commit

.Docs
* {latest-docs}/core/[HTTP API]
* {latest-docs}/schema/config/full-config.html[Full Stack Config]
* {latest-docs}/schema/config/plugin-config.html[Plugin Config]

.Schema
* {latest-docs}/schema/config/full-config.json[Service Stack Configuration Schema]
* {latest-docs}/schema/config/plugin-config.json[Plugin-Specific Configuration Schema] +
...
be used in filtering the logs to only lines relevant to a specific task.

The keys are constants link:/project/common/logging/src/main/kotlin/vdi/logging/extensions.kt[defined here].
...
VDI is an event driven asynchronous system where a single event can result in
actions across multiple services.  To help trace the path and results of an
event, a numeric ID is assigned to each event that will follow the processing of
that event through each part of the core VDI service and plugin services.
