...
internal class DatasetShareOfferFileImpl(path: String,
                                         existsChecker: () -> Boolean,
                                         lastModifiedSupplier: () -> OffsetDateTime?,
                                         loadObjectStream: () -> InputStream?)
: DatasetFileImpl(S3Paths.SHARE_OFFER_FILE_NAME, path, existsChecker, lastModifiedSupplier, loadObjectStream)
, DatasetShareOfferFile
{
    ...
    constructor(s3Object: S3Object): this(
        ...
        if (s3Object.baseName != S3Paths.SHARE_OFFER_FILE_NAME) {
            throw IllegalArgumentException("Can only construct a MetaFile from s3 object if object base name is "
                    + S3Paths.SHARE_OFFER_FILE_NAME)
        }
    }

  override fun load(): VDIDatasetShareOffer? = loadContents()?.use { JSON.readValue(it) }
}