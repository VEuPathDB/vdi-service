...
. Client makes a multipart `POST` request with the metadata and dataset files to
  be imported.
. Service sanity checks metadata
.. Check dataset type matches something we can import
.. Check target project IDs exist
. Service generates a dataset ID
. Service pushes uploaded data to a temp store
.. What is this temp store?
. Service creates a record for the import in the PostgreSQL database with the
  status awaiting import
. Service queues the dataset to be imported.
. Service returns the dataset ID to the client
