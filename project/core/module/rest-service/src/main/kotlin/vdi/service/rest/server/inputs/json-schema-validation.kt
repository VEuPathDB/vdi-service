...
fun ObjectNode.validate(schema: Schema, errors: ValidationErrors) {
  val result = schema.validate(this) { ctx: ExecutionContext -> ctx.executionConfig {
  ...
  val furtherProcessing = HashMap<String, MutableList<Error>>(8)
  ...
  result.forEach { msg ->
    ...
    // TODO: hack until new API is better understood/explored
    if (!msg.isValid)
      errors.add(path, msg.message)

//    when (msg.schemaNode.get("type").textValue()) {
//      null -> ???
//      ""
