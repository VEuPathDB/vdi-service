...
interface AppDBAccessor {
   ...
   */
  fun streamAllSyncControlRecords(): CloseableIterator<ReconcilerTargetRecord>
   ...
   * tested for.
   *
   * @param installTarget ID of the target project that the link record should
   * match.
   *
   * @return `true` if a matching dataset-to-project link record was found,
   * otherwise `false`.
   */
  fun testDatasetProjectLinkExists(datasetID: DatasetID, installTarget: InstallTargetID): Boolean
}
