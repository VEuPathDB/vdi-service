 ...
 */
internal class DatasetDirIterator(private val bucket: S3Bucket, private val objectStream: Iterator<S3Object>)
    ...
    while (true) {
      ...
      if (!objectStream.hasNext()) {
        ...
        if (stagedObjects.isNotEmpty()) {
          ...
          try {
            return MaterializedDatasetDirectory.build(bucket, stagedObjects, userID, datasetID, pathFactory)
      ...
      if (currDatasetID == nextObjectDatasetID) {
        ...
        try {
          ...
          return MaterializedDatasetDirectory.build(bucket, stagedObjects, currUserID, currDatasetID, pathFactory)
        } catch (e: MalformedDatasetException) {
